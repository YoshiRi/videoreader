...
for i = 1:Numframes-1
          val_diff(i,:) = (val_ref(i+1,:) - val_ref(i,:)) .* vidObj.FrameRate;
          if i == 1
              val_diff2(i,:) =  (val_ref(i+1,:) -[0,0,1,0]) .* (1/2 *vidObj.FrameRate);
          else
              val_diff2(i,:) =  (val_ref(i+1,:) - val_ref(i-1,:)) .* (1/2 *vidObj.FrameRate);
          end
end
...
LPval_diff = filtfilt(b,a,val_diff2);

%  ÉçÅ[ÉpÉX
N   = 10;        % FIR filter order
Fp  = 1/20;       % 30 Hz passband-edge frequency
Fs  = vidObj.FrameRate;       % sampling frequency
...
DFTandShow(time,val_diff2(:,2));
DFTandShow(time,val_diff2(:,3));
DFTandShow(time,val_diff2(:,4));

%% Print to file
% time refnum ref1 ref2 
% fileID = fopen('refdata1.txt','w');
output= horzcat(time(1:Numframes-1,1), refnum(1:Numframes-1,1),  val_ref(1:Numframes-1,1),val_ref(1:Numframes-1,2),val_ref(1:Numframes-1,3),val_ref(1:Numframes-1,4)   ,LPval_diff2(:,1),LPval_diff2(:,2),LPval_diff2(:,3),LPval_diff2(:,4));
fname = strsplit(Filename,'.');
outputfile = cell2mat(strcat(fname(1),'.txt'))  
dlmwrite(outputfile,output); 

%% save workspace
outputfile2 = cell2mat(strcat(fname(1),'.mat'))    
save(outputfile2);