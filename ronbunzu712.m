...
scolor = struct('cdata',zeros(vidHeight,vidWidth,3,'uint8'),...
    'colormap',[]);
k = 1;
...
while hasFrame(vidObj)
   scolor(k).cdata = imcrop(readFrame(vidObj),[ cx - Wid/2, cy - Hei/2,Wid-1,Hei-1]) ;
   k = k+1;
end

imwrite(scolor(1).cdata,'ref_1c.png');
imwrite(scolor(98).cdata,'ref_98c.png');
imwrite(scolor(178).cdata,'ref_178c.png');
imwrite(scolor(269).cdata,'ref_269c.png');
% % % Put value
% % Numframes = k - 1;
% % initialize
...
% %     [Xi mpeak] = RIPOC_func(RF,scolor(i).cdata);
% %     hoge
...
% %         end
% %     else
% %         [Xi mpeak] = RIPOC_func(scolor(i-1).cdata,scolor(i).cdata);
